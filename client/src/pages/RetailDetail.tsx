...
import { MapPin, DollarSign, CheckCircle2, ArrowLeft, Share2, Phone, Mail, Ruler, Building } from "lucide-react";
...
export default function RetailDetail() {
  const [, params] = useRoute("/retail/:id");
  const [listing, setListing] = useState<RetailListing | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/data/retail.json')
      ...
      .then(data => {
        const found = data.find((item: RetailListing) => item.id === params.id);
        ...
        <div className="container flex items-center justify-between">
          <Link href="/office-retail">
            <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-primary pl-0">
              <ArrowLeft className="w-4 h-4 mr-2" /> 返回办公商铺列表
            </Button>
          </Link>
